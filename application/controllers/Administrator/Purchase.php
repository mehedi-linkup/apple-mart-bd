...
class Purchase extends CI_Controller
{
    ...
    public function  deletePurchase(){
        ...
        try{
            ...
            // check sale
            $countSale = $this->db->query("
                select 
                    ps.*
                from tbl_product_serial_numbers ps 
                where ps.ps_purchase_inv = ?
                and ps.ps_s_status = 'yes'
                and ps.ps_brunch_id = ?
            ", [$purchase->PurchaseMaster_InvoiceNo, $this->session->userdata('BRANCHid')])->num_rows();

            if($countSale > 0) {
                $res = ['success'=>false, 'message'=>'Unable to delete. Product sale is is found'];
                echo json_encode($res);
                exit;
            }
    ...
    function addDamage()
    {
        ...
        try {
            ...
            $damageDetailId = $this->db->insert_id();
            ...
            $this->db->query("
                ...
                set ps_dm_status = 'yes',
                damage_detail_id = ?
                where ps_id = ?
                and ps_serial_number = ?
            ", [$damageDetailId, $data->ps_id, $data->damage_seriral]);
    ...
    public function updateDamage(){
        ...
        try {
            ...
            // remove product serial
            $serialProduct = $this->db->query("select * from tbl_product_serial_numbers where damage_detail_id = ?", $oldProduct->DamageDetails_SlNo);
            if($serialProduct->num_rows() > 0) {
                $oldSerial = $serialProduct->row();

                $this->db->query("
                    update  tbl_product_serial_numbers
                    set ps_dm_status = 'no',
                    damage_detail_id = null
                    where damage_detail_id = ?
                ", $oldProduct->DamageDetails_SlNo);
            }
            ...
            $this->db->query("
                ...
                where ps_id = ?
                and ps_serial_number = ?
            ", [$oldProduct->DamageDetails_SlNo, $data->ps_id, $data->damage_seriral]);
    ...
    public function getDamages(){
        ...
        $damages = $this->db->query("
            ...
            left join tbl_product_serial_numbers ps on ps.damage_detail_id = dd.DamageDetails_SlNo
