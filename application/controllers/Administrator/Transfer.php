    ...
    class Transfer extends CI_Controller{
        ...
        public function addProductTransfer(){
            ...
            try{
                ...
                foreach($data->cart as $cartProduct){
                    ...
                    $transDetailsId = $this->db->insert_id();
        ...
        public function updateProductTransfer(){
            ...
            try{
                ...
                $oldTransSerial = $this->db->query("select ps.* from tbl_product_serial_numbers ps where ps.transfer_detail_id = ?", $transferId)->result();
                ...
                foreach($oldTransSerial as $s) {
                    ...
                    $this->db->where('ps_id', $s->ps_id)->where('ps_prod_id', $s->ps_prod_id)->update('tbl_product_serial_numbers', $serialUpdate);
                }
        ...
        public function getTransferDetails() {
            ...
            $transferDetails = array_map(function($product) {
                $product->serials = $this->db->query("
                    ...
                    and ps.transfer_detail_id = ?
                ", [$product->product_id, $product->transfer_to, $product->transfer_id])->result();
                return $product;
            }, $transferDetails);            
        ...
        public function transferInvoice($transferId){
            ...
            $transferDetails = array_map(function($trans) {
                $trans->serials = $this->db->query("
                    select 
                        ps.*
                    from tbl_product_serial_numbers ps 
                    where ps.ps_status = 'a'
                    and ps.transfer_detail_id = ?
                    and ps.ps_prod_id = ?
                ", [$trans->transfer_id, $trans->product_id])->result();
                return $trans;
            }, $transferDetails);
        ...
        public function deleteTransfer() {
            ...
            try{
                ...
                foreach($oldTransferDetails as $oldDetails) {
                    ...
                    $this->db->where('transfer_detail_id', $transferId)->where('ps_prod_id', $oldDetails->product_id)->update('tbl_product_serial_numbers', $serial);
                }
